...
export default function BookingConfirmationPage() {
  ...
  if (!state?.booking) {
    return (
      <div className="text-center py-16">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No confirmation data</h2>
        <p className="text-gray-500 dark:text-slate-400 mb-6">
          This page must be reached after completing a booking.
        </p>
        <Link to="/passenger/bookings" className="btn-gradient">My Bookings</Link>
      </div>
    );
  }
  ...
  const payFailed   = payment?.status === 'FAILED';
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className={`rounded-2xl p-6 mb-6 flex items-start gap-4 ${
        ...
        <div className={`text-4xl ${isConfirmed ? '' : ''}`}>
          {isConfirmed ? '✅' : '❌'}
        </div>
        <div>
          <h1 className={`text-xl font-bold mb-1 ${
            ...
            {isConfirmed ? 'Booking confirmed!' : 'Payment failed'}
          </h1>
          <p className={`text-sm ${
            isConfirmed
      ...
      <div className="card mb-5 bg-[#f0ebff] dark:bg-[#1a1035] border border-[#c4b5fd] dark:border-[#4c1d95]">
        <p className="text-xs text-gray-500 dark:text-slate-400 mb-1">Booking reference</p>
      ...
      <div className="card mb-5">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-gray-400 dark:text-slate-500 mb-1">Booking status</p>
            <StatusBadge status={booking.status} />
          </div>
          {payment && (
            <div>
              <p className="text-gray-400 dark:text-slate-500 mb-1">Payment status</p>
      ...
      {sch && (
        <div className="card mb-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-gray-400 dark:text-slate-500">Seats booked</p>
